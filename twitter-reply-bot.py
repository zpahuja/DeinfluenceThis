...
# Load Twitter and Airtable API keys from environment variables
...
class TwitterBot:
    def __init__(self):
        ...
        # Initialize the language model with temperature of .5 to induce some creativity
        self.llm = ChatOpenAI(temperature=.5, openai_api_key=OPENAI_API_KEY, model_name='gpt-4')

        # For statistics tracking for each run. This is not persisted anywhere, just for logging
    ...
    def generate_response(self, mentioned_conversation_tweet_text):
        ...
        Please provide such opinion with the original opinion below:
    ...
    def get_mentions(self):
        # TODO If doing this in prod make sure to deal with pagination. There could be a lot of mentions!
        now = datetime.utcnow()
